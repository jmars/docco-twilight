(function() {
  var destination, docco_styles, docco_template, ensure_directory, exec, ext, files, fs, generate, generate_documentation, generate_html, get_language, highlight, highlight_end, highlight_start, l, languages, parse, path, read_source, settings, showdown, source_file, spawn, template, _ref;
  ...
  read_source = function(filepath, callback) {
    return fs.stat(filepath, function(err, stats) {
      var child_files;
      if (stats.isDirectory()) {
        child_files = fs.readdirSync(filepath);
      }
      return callback(child_files);
    });
  };
    ...
    if (targets.length) {
      return ensure_directory('docs', function() {
        ...
        next_file = function() {
          var file;
          file = files.shift();
          return read_source(file, function(child_files) {
            if (child_files) {
              files = files.concat(child_files.map(function(child) {
                return path.join(file, child);
              }));
              if (child_files.indexOf('controller' !== -1)) {
                require('eyes').inspect(files);
              }
              return next_file();
            }
            if (files.length) {
              return generate_documentation(file, next_file);
            }
          });
        };
        return next_file();
